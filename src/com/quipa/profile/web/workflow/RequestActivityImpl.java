...
public class RequestActivityImpl implements RequestActivity {
	...
	public List<RequestRepresentation> getRequests(Long profileId) {
		...
		requests = requestService.listAll(profileId);
	...
	public RequestRepresentation createRequest(RequestRequest requestRequest) {
		Request request = new Request(null,AppBaseUtilsWeb.StringToCalendar(requestRequest.getDate(), AppBaseConstantsWeb.DATE_FORMAT),
				requestRequest.getFromHour(),requestRequest.getToHour(),requestRequest.getHours(),requestRequest.getPriceHour()
				,requestRequest.getSubTotal(),requestRequest.getTaxes(),requestRequest.getTransportation(),requestRequest.getTotal()
				,requestRequest.getRequiredSkill(),requestRequest.getJobTitle(),requestRequest.getDescription(),AppBaseConstantsWeb.REQUEST_STATUS_PENDING,
				profileService.getById(requestRequest.getProfileId()),profileService.getById(requestRequest.getProspectId()));
